...
export function getCanvasContainerSize(canvas: CompatibleCanvas): { width: number; height: number } {
  // Audio and Timelines without a spatial extent must not resize the Atlas world.
  return {
    width: typeof canvas.width === 'number' && canvas.width > 0 ? canvas.width : 0,
    height: typeof canvas.height === 'number' && canvas.height > 0 ? canvas.height : 0,
  };
}
