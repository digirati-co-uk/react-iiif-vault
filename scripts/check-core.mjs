...
const forbidden = /^(?:react-dom|react-reconciler|preact|@atlas-viewer\/atlas|@floating-ui|polygon-editor)(?:\/|$)/;
...
for (const format of ['import', 'require']) {
  ...
  const module = format === 'import' ? await import(pathToFileURL(resolve(root, target))) : require(pkg.name + '/core');
...
console.log('Passed: core ESM/CJS dependency graphs and shared runtime/context identity.');
