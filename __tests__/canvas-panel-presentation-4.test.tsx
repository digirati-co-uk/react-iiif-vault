import { useExternalManifest, useManifest, useCanvas, useAnnotation, usePaintingAnnotations, useCanvasSubset } from '../src/presentation-4';
/**
...
describe('CanvasPanel Presentation 4 compatibility', () => {
  test('treats a v4 Timeline as a non-spatial CanvasPanel view', () => {
    ...
    expect(getCanvasContainerSize(timeline)).toEqual({ width: 0, height: 0 });
...
test('retains Scene data but excludes it from CanvasPanel rendering and sequencing', () => {
  const vault = new Vault();
  const scene = { id: 'https://example.org/scene', type: 'Scene', items: [] };
  const loaded = vault.loadManifestSync('https://example.org/scenes', {
    '@context': 'http://iiif.io/api/presentation/4/context.json',
    id: 'https://example.org/scenes', type: 'Manifest', items: [scene],
  })!;
  const resource = vault.get({ id: scene.id, type: 'Scene' });
  expect(resource.type).toBe('Scene');
  expect(getContainerSequence(vault, loaded)[0]).toEqual([]);
  expect(getRenderingStrategy({ canvas: resource, paintables: createPaintingAnnotationsHelper(vault).getPaintables([]), supports: ['empty', '3d-model'], loadImageService: () => undefined, vault })).toMatchObject({ type: 'unknown', reason: 'Scene rendering is not supported' });
  const view = render(<VaultProvider vault={vault}><CanvasContext canvas={scene.id}><CanvasPanel.CanvasBackground /></CanvasContext></VaultProvider>);
  expect(view.container.childElementCount).toBe(0);
});


test('v4 entry-point hooks load and select native fixture resources', async () => {
  const vault = new Vault({ customFetcher: async () => manifest });
  const { result } = renderHook(() => ({
    request: useExternalManifest(manifestId),
    manifest: useManifest(),
    canvas: useCanvas(),
    annotation: useAnnotation({ id: `${canvasId}/painting` }),
    painting: usePaintingAnnotations(),
    subset: useCanvasSubset(),
  }), { wrapper: ({ children }) => <VaultProvider vault={vault} resources={{ manifest: manifestId, canvas: canvasId }}>{children}</VaultProvider> });
  await waitFor(() => expect(result.current.request.isLoaded).toBe(true));
  expect(result.current.manifest).toEqual(vault.get({ id: manifestId, type: 'Manifest' }));
  expect(result.current.canvas?.backgroundColor).toBe('#123456');
  expect(result.current.annotation).toEqual(vault.get({ id: `${canvasId}/painting`, type: 'Annotation' }));
  expect(result.current.painting).toEqual([result.current.annotation]);
  expect(result.current.subset).toEqual([{ id: canvasId, type: 'Canvas' }]);
});
